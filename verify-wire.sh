...
python3 - <<PY
import json
m2={"v":1,"realm":"ftf","topic":"provenance/demo","prev":"$TRACE_MH1","t":2,"author":"$TRACE_AUTHOR","caps":[],"body":{"kind":"xform","tool":{"id":"$TRACE_DUMMY","version":"v1","params_hash":"$TRACE_DUMMY"},"inputs":["$TRACE_A"],"outputs":["$TRACE_B"],"receipt":{"recipe_hash":"$TRACE_DUMMY","env_hash":"$TRACE_DUMMY"}},"witness":None,"sig":""}
...
if ! grep -q "\"kind\":\"trace.header\".*\"root\":\"$TRACE_B\"" "$TRACE_OUTPUT"; then
  printf '[verify-wire] trace header root mismatch\n' >&2
  fail_trace
fi
if ! grep -q "\"h\":\"$TRACE_B\".*\"status\":\"ok\"" "$TRACE_OUTPUT"; then
...
if ! grep -q "\"kind\":\"trace.edge\".*\"rel\":\"produces\".*\"to\":\"artifact:$TRACE_B\"" "$TRACE_OUTPUT"; then
